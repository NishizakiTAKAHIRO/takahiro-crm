...
const STATUS_COLOR = { "初回コンタクト": "#94a3b8", "提案済": "#60a5fa", "商談中": "#f59e0b", "契約済": "#22c55e", "失注": "#ef4444", "活動中": "#60a5fa", "マッチング中": "#f59e0b", "成約済": "#22c55e", "未アプローチ": "#94a3b8", "交渉中": "#f59e0b", "打診済": "#60a5fa" };
    ...
    <div>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 24 }}>
        {categories.map(cat => {
          ...
          return <Card key={cat} title={`${cat} 達成率`} value={`${avgPct}%`} sub={`${items.length}KPI`} color={CAT_COLOR[cat] || "#64748b"} icon={cat === "RA営業" ? "🏢" : cat === "CA" ? "👤" : cat === "成果" ? "🎯" : "💰"} />;
        })}
      </div>
      {categories.map(cat => (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      {editing?.id === k.id && editing?.field === "actual"
                        ? <div style={{ display: "flex", gap: 4 }}><input type="number" value={editVal} onChange={e => setEditVal(e.target.value)} onKeyDown={e => { if (e.key === "Enter") saveEdit(); if (e.key === "Escape") setEditing(null); }} autoFocus style={{ width: 80, padding: "3px 6px", borderRadius: 6, border: "2px solid #2563eb", fontSize: 13 }} /><button onClick={saveEdit} style={{ padding: "3px 8px", background: "#2563eb", color: "#fff", border: "none", borderRadius: 6, fontSize: 12, cursor: "pointer" }}>✓</button></div>
                        : <div onClick={() => startEdit(k.id, "actual", k.actual)} style={{ fontSize: 22, fontWeight: 800, color, cursor: "pointer", display: "flex", alignItems: "baseline", gap: 2 }}>{isMoney ? `¥${k.actual.toLocaleString()}` : k.actual}<span style={{ fontSize: 12, color: "#94a3b8" }}>{!isMoney && k.unit}</span><span style={{ fontSize: 11, color: "#cbd5e1", marginLeft: 4 }}>✏️</span></div>}
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 2 }}>目標</div>
                      {editing?.id === k.id && editing?.field === "target"
                        ? <div style={{ display: "flex", gap: 4 }}><input type="number" value={editVal} onChange={e => setEditVal(e.target.value)} onKeyDown={e => { if (e.key === "Enter") saveEdit(); if (e.key === "Escape") setEditing(null); }} autoFocus style={{ width: 80, padding: "3px 6px", borderRadius: 6, border: "2px solid #94a3b8", fontSize: 13 }} /><button onClick={saveEdit} style={{ padding: "3px 8px", background: "#64748b", color: "#fff", border: "none", borderRadius: 6, fontSize: 12, cursor: "pointer" }}>✓</button></div>
                        : <div onClick={() => startEdit(k.id, "target", k.target)} style={{ fontSize: 16, fontWeight: 700, color: "#64748b", cursor: "pointer" }}>{isMoney ? `¥${k.target.toLocaleString()}` : `${k.target}${k.unit}`}<span style={{ fontSize: 11, color: "#cbd5e1", marginLeft: 4 }}>✏️</span></div>}
                    </div>
      ...
      <div style={{ background: "#f8fafc", borderRadius: 10, padding: 12, marginTop: 8, fontSize: 12, color: "#94a3b8", textAlign: "center" }}>💡 実績・目標の数字をクリックすると編集できます</div>
    </div>
  );
}

const TABS = [{ id: "dashboard", label: "📊 ダッシュボード" }, { id: "kimero", label: "👔 キメロコスメ" }, { id: "smile", label: "🍱 スマイル&ナリッシュ" }, { id: "huppy", label: "🎵 フーピー" }, { id: "tasks", label: "🔥 TODAY" }];
...
function Dashboard({ data }) {
  ...
  const bizData = [{ name: "フーピー", 売上: huppyCurrent.total, 個人報酬: huppyCurrent.personal }, { name: "スマイル", 売上: smileMonthly, 個人報酬: 0 }, { name: "キメロ", 売上: 0, 個人報酬: 0 }];
  const goalData = [{ name: "フーピー", 現在: huppyCurrent.personal, 目標: 500000 }, { name: "キメロ", 現在: 0, 目標: 400000 }, { name: "スマイル", 現在: 0, 目標: 150000 }];
  return (
    <div>
      <div style={{ marginBottom: 24 }}><h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#1e293b" }}>全社ダッシュボード</h2><p style={{ margin: "4px 0 0", color: "#64748b", fontSize: 13 }}>月収100万円達成ロードマップ</p></div>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
        ...
        <Card title="スマイル今月売上" value={`${(smileMonthly/10000).toFixed(1)}万円`} sub={`${data.smile.sales.reduce((s,d)=>s+d.shoku,0)}食`} color="#16a34a" icon="🍱" />
        <Card title="キメロ KPI達成率" value={`${kpiAvg}%`} sub={`${data.kimero.kpi.length}KPI`} color={kpiAvg >= 80 ? "#22c55e" : kpiAvg >= 50 ? "#f59e0b" : "#ef4444"} icon="🎯" />
        <Card title="今日のTASK" value={`${taskDone}/${data.tasks.length}`} sub={`${Math.round(taskDone/data.tasks.length*100)}%`} color="#ef4444" icon="🔥" />
      </div>
          ...
          {data.kimero.kpi.map(k => {
            ...
            return (
              <div key={k.id} style={{ padding: "10px 14px", background: "#f8fafc", borderRadius: 10, border: "1px solid #e2e8f0" }}>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>{k.unit === "円" ? `¥${k.actual.toLocaleString()} / ¥${k.target.toLocaleString()}` : `${k.actual} / ${k.target}${k.unit}`}</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, boxShadow: "0 1px 6px rgba(0,0,0,0.08)" }}>
          ...
          <ResponsiveContainer width="100%" height={180}><BarChart data={bizData} barSize={28}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" /><XAxis dataKey="name" tick={{ fontSize: 12 }} /><YAxis tick={{ fontSize: 11 }} tickFormatter={v => `${(v/10000).toFixed(0)}万`} /><Tooltip formatter={v => `¥${v.toLocaleString()}`} /><Bar dataKey="売上" fill="#2563eb" radius={[4,4,0,0]} /><Bar dataKey="個人報酬" fill="#22c55e" radius={[4,4,0,0]} /></BarChart></ResponsiveContainer>
        </div>
        <div style={{ background: "#fff", borderRadius: 12, padding: 20, boxShadow: "0 1px 6px rgba(0,0,0,0.08)" }}>
          <h4 style={{ margin: "0 0 16px", fontSize: 13, color: "#475569", fontWeight: 700 }}>目標達成率</h4>
          {goalData.map(g => { const pct = Math.min(100, Math.round(g.現在 / g.目標 * 100)); return (<div key={g.name} style={{ marginBottom: 14 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}><span style={{ fontWeight: 600 }}>{g.name}</span><span style={{ color: "#64748b" }}>¥{g.現在.toLocaleString()} / ¥{g.目標.toLocaleString()}</span></div><div style={{ background: "#f1f5f9", borderRadius: 6, height: 10, overflow: "hidden" }}><div style={{ width: `${pct}%`, height: "100%", background: BIZ_COLOR[g.name] || "#2563eb", borderRadius: 6 }} /></div><div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{pct}%</div></div>); })}
        </div>
      </div>
    </div>
  );
...
function Kimero({ data, setData }) {
  ...
  function addContact() { if (!form.company) return; setData(d => ({ ...d, kimero: { ...d.kimero, contacts: [...d.kimero.contacts, { ...form, id: Date.now() }] } })); setForm({ company: "", person: "", type: "人材派遣", status: "初回コンタクト", date: new Date().toISOString().split("T")[0], note: "" }); }
  const statusCount = ["初回コンタクト","提案済","商談中","契約済"].map(s => ({ status: s, count: data.kimero.contacts.filter(c => c.status === s).length }));
  return (
    <div>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20 }}>{statusCount.map(s => <Card key={s.status} title={s.status} value={`${s.count}件`} color={STATUS_COLOR[s.status] || "#64748b"} icon="" />)}</div>
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {["kpi","contacts","seekers","revenue"].map(t => <button key={t} onClick={() => setTab(t)} style={{ padding: "6px 16px", borderRadius: 20, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 12, background: tab === t ? "#2563eb" : "#f1f5f9", color: tab === t ? "#fff" : "#475569" }}>{t === "kpi" ? "🎯 KPI管理" : t === "contacts" ? "🏢 企業コンタクト" : t === "seekers" ? "👤 求職者" : "📈 売上推移"}</button>)}
      </div>
      ...
      {tab === "contacts" && (
        <Section title="企業コンタクト管理" color="#2563eb">
          <div style={{ background: "#f8fafc", borderRadius: 10, padding: 16, marginBottom: 16, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "flex-end" }}>
            ...
            <div><div style={{ fontSize: 11, color: "#64748b", marginBottom: 4 }}>種別</div><select value={form.type} onChange={e => setForm(f=>({...f,type:e.target.value}))} style={{ padding: "6px 10px", borderRadius: 6, border: "1px solid #e2e8f0", fontSize: 13 }}>{["人材派遣","職業紹介","業務委託","BPO"].map(t => <option key={t}>{t}</option>)}</select></div>
            <div><div style={{ fontSize: 11, color: "#64748b", marginBottom: 4 }}>ステータス</div><select value={form.status} onChange={e => setForm(f=>({...f,status:e.target.value}))} style={{ padding: "6px 10px", borderRadius: 6, border: "1px solid #e2e8f0", fontSize: 13 }}>{["初回コンタクト","提案済","商談中","契約済","失注"].map(s => <option key={s}>{s}</option>)}</select></div>
          ...
          <Table headers={["会社名","担当者","種別","ステータス","日付","メモ"]} rows={data.kimero.contacts.map(c => [<span style={{ fontWeight: 600 }}>{c.company}</span>, c.person, <Badge label={c.type} color="#2563eb" />, <Badge label={c.status} color={STATUS_COLOR[c.status]} />, c.date, c.note])} />
        </Section>
      )}
      {tab === "seekers" && <Section title="求職者データベース" color="#2563eb"><Table headers={["氏名","スキル・経験","ステータス","希望雇用形態","メモ"]} rows={data.kimero.seekers.map(s => [<span style={{ fontWeight: 600 }}>{s.name}</span>, s.skill, <Badge label={s.status} color={STATUS_COLOR[s.status]} />, s.desired, s.note])} /></Section>}
      {tab === "revenue" && <Section title="売上目標 vs 実績" color="#2563eb"><ResponsiveContainer width="100%" height={220}><BarChart data={data.kimero.monthlyRevenue}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" /><XAxis dataKey="month" /><YAxis tickFormatter={v => `${(v/10000).toFixed(0)}万`} /><Tooltip formatter={v => `¥${v.toLocaleString()}`} /><Bar dataKey="target" fill="#dbeafe" radius={[4,4,0,0]} name="目標" /><Bar dataKey="actual" fill="#2563eb" radius={[4,4,0,0]} name="実績" /></BarChart></ResponsiveContainer></Section>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <Section title="売上報告入力（LINEから転記）" color="#16a34a">
        ...
        <Table headers={["日付","スタッフ","食数","現金","PayPay","合計"]} rows={data.smile.sales.map(s => [s.date, s.staff, `${s.shoku}食`, `¥${s.cash.toLocaleString()}`, `¥${s.paypay.toLocaleString()}`, <span style={{ fontWeight: 700, color: "#16a34a" }}>¥{(s.cash+s.paypay).toLocaleString()}</span>])} />
      </Section>
    </div>
  );
}
...
function Huppy({ data }) {
  ...
  return (
    <div>
      ...
      <Section title="月次推移" color="#9333ea"><ResponsiveContainer width="100%" height={200}><BarChart data={data.huppy.revenue}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" /><XAxis dataKey="month" /><YAxis tickFormatter={v => `${(v/10000).toFixed(0)}万`} /><Tooltip formatter={v => `¥${v.toLocaleString()}`} /><Bar dataKey="total" fill="#e9d5ff" radius={[4,4,0,0]} name="売上" /><Bar dataKey="personal" fill="#9333ea" radius={[4,4,0,0]} name="個人報酬" /></BarChart></ResponsiveContainer></Section>
      <Section title="パートナー・案件管理" color="#9333ea"><Table headers={["パートナー名","種別","ステータス","想定金額","メモ"]} rows={data.huppy.partners.map(p => [<span style={{ fontWeight: 600 }}>{p.name}</span>, p.type, <Badge label={p.status} color={STATUS_COLOR[p.status]} />, p.value, p.note])} /></Section>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ background: "#fff", borderRadius: 12, padding: 20, marginBottom: 20, boxShadow: "0 1px 6px rgba(0,0,0,0.08)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>🔥 今日のTASK</h3><span style={{ fontSize: 24, fontWeight: 800, color: pct === 100 ? "#22c55e" : "#2563eb" }}>{pct}%</span></div>
        ...
        {data.tasks.map(t => (
          <div key={t.id} onClick={() => toggle(t.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderRadius: 10, marginBottom: 8, cursor: "pointer", background: t.done ? "#f0fdf4" : "#f8fafc", border: `1px solid ${t.done ? "#bbf7d0" : "#e2e8f0"}` }}>
            <div style={{ width: 22, height: 22, borderRadius: 6, border: `2px solid ${t.done ? "#22c55e" : "#cbd5e1"}`, background: t.done ? "#22c55e" : "white", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{t.done && <span style={{ color: "white", fontSize: 13, fontWeight: 800 }}>✓</span>}</div>
        ...
        {pct === 100 && <div style={{ textAlign: "center", padding: 16, color: "#22c55e", fontWeight: 800, fontSize: 16 }}>🎉 今日のタスク全完了！お疲れ様でした！</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ fontFamily: "'Hiragino Sans', 'Yu Gothic', Arial, sans-serif", background: "#f1f5f9", minHeight: "100vh" }}>
      <div style={{ background: "#1e3a5f", padding: "14px 24px", display: "flex", alignItems: "center", gap: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>
        <div><div style={{ color: "#fff", fontWeight: 800, fontSize: 18, letterSpacing: 0.5 }}>UCHIWA_CRM</div><div style={{ color: "#93c5fd", fontSize: 11, marginTop: 1 }}>月収100万円達成ダッシュボード</div></div>
